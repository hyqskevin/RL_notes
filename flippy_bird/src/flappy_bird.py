...
class FlappyBird(object):
    ...
    def is_collided(self):
        # check if the bird touch the top
        if self.bird_y <= 1:
            return True
    ...
    def next_frame(self, action):
        ...
        for pipe in self.pipes:
            ...
            pipe_center_y_lower = pipe['y_lower'] / 2
            # pipe_center_y_upper = - pipe['y_upper']
            # print(pipe_center_y_lower * 1.5 + 20, bird_center_y)
            if pipe_center_x < bird_center_x < pipe_center_x + 5:
                self.score += 1
                reward = 1 * abs(pipe_center_x - bird_center_x)
                break
            # 限制在水管中间
            # elif (pipe_center_y_lower * 1.5 + 40) < bird_center_y < (pipe_center_y_lower * 1.5 + 60):
            elif (pipe_center_y_lower * 1.5 + self.bird_height + 5) < \
                    bird_center_y < (pipe_center_y_lower * 1.5 + 3* self.bird_height):
                self.score += 0.5
                reward = 1 * abs(pipe_center_y_lower * 1.5 + 50 - bird_center_y)
                break
            else:
                self.score = 0
                reward = -0.01 * abs(pipe_center_y_lower* 1.5 + 50 - bird_center_y)
                break
